...
enum {
   ...
   trace_buffer_size = trace_element_count * sizeof(uint32_t),
   trace_salt = 0x5a17u,
};
...
main(void)
{
   static NSString *const source =
      @"#include <metal_stdlib>\n"
       ...
       "{\n"
       "   output[index] = input[index] ^ (salt + index * 13u);\n"
       "}\n";
   id<MTLDevice> device;
   ...
   @autoreleasepool {
      if (!verify_agx_device_reopen()) {
         fputs("AO46_AGX_COMPUTE_RECORD device profile gate failed\n", stderr);
         return 1;
      }

      device = MTLCreateSystemDefaultDevice();
      if (!device) {
         fputs("AO46_AGX_COMPUTE_RECORD no Metal device\n", stderr);
         return 1;
      }

      library = [device newLibraryWithSource:source options:nil error:&error];
      if (!library) {
         fprintf(stderr, "AO46_AGX_COMPUTE_RECORD shader compile failed: %s\n",
      ...
      if (!function) {
         fputs("AO46_AGX_COMPUTE_RECORD shader entry point missing\n", stderr);
         return 1;
      }

      pipeline = [device newComputePipelineStateWithFunction:function error:&error];
      if (!pipeline) {
         fprintf(stderr, "AO46_AGX_COMPUTE_RECORD pipeline creation failed: %s\n",
                 error ? error.localizedDescription.UTF8String : "unknown error");
         return 1;
      }

      queue = [device newCommandQueue];
      puts("AO46_AGX_COMPUTE_RECORD allocate input-shared");
      input = [device newBufferWithLength:trace_buffer_size
                                   options:MTLResourceStorageModeShared];
      puts("AO46_AGX_COMPUTE_RECORD allocate output-shared");
      ...
      if (!queue || !input || !output ||
         ...
         fputs("AO46_AGX_COMPUTE_RECORD resource setup failed\n", stderr);
         return 1;
      }
      ...
      for (unsigned i = 0; i < trace_element_count; ++i) {
         if (output_values[i] != (input_values[i] ^ (trace_salt + i * 13u))) {
            fputs("AO46_AGX_COMPUTE_RECORD output verification failed\n", stderr);
            return 1;
         }
      }

      printf("AO46_AGX_COMPUTE_RECORD complete elements=%u bytes=%u\n",
             trace_element_count, trace_buffer_size);
