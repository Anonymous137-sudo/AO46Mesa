...
main(void)
{
   ...
   @autoreleasepool {
      if (!verify_agx_device_reopen()) {
         fputs("AO46_AGX_RENDER_RECORD device profile gate failed\n", stderr);
         return 1;
      }

      device = MTLCreateSystemDefaultDevice();
      if (!device) {
         fputs("AO46_AGX_RENDER_RECORD no Metal device\n", stderr);
         return 1;
      }

      library = [device newLibraryWithSource:source options:nil error:&error];
      if (!library) {
         fprintf(stderr, "AO46_AGX_RENDER_RECORD shader compile failed: %s\n",
      ...
      if (!vertex || !fragment) {
         fputs("AO46_AGX_RENDER_RECORD shader entry point missing\n", stderr);
         return 1;
      }
      ...
      if (!pipeline) {
         fprintf(stderr, "AO46_AGX_RENDER_RECORD pipeline creation failed: %s\n",
                 error ? error.localizedDescription.UTF8String : "unknown error");
         return 1;
      }
      ...
      puts("AO46_AGX_RENDER_RECORD allocate color-target-rgba8");
      color_target = [device newTextureWithDescriptor:texture_descriptor];
      if (!queue || !color_target) {
         fputs("AO46_AGX_RENDER_RECORD resource setup failed\n", stderr);
         return 1;
      }

      puts("AO46_AGX_RENDER_RECORD render fullscreen-triangle-to-color-target");
      if (!submit_render(queue, pipeline, color_target)) {
         fputs("AO46_AGX_RENDER_RECORD render submission failed\n", stderr);
         return 1;
      }
      ...
      for (size_t offset = 0; offset < sizeof(readback);
         ...
         if (!is_expected_color(readback[offset], 64) ||
            ...
            fputs("AO46_AGX_RENDER_RECORD color verification failed\n", stderr);
            return 1;
         }
      }

      printf("AO46_AGX_RENDER_RECORD complete width=%u height=%u bytes=%u\n",
