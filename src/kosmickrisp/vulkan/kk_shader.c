...
static nir_def *
kk_query_custom_border(nir_builder *b, nir_tex_instr *tex)
{
...
static nir_def *
kk_has_custom_border(nir_builder *b, nir_tex_instr *tex)
{
...
static bool
kk_lower_custom_border_tex(nir_builder *b, nir_tex_instr *tex, UNUSED void *data)
{
   ...
   nir_def *has_custom = kk_has_custom_border(b, tex);

   nir_instr *original = nir_instr_clone(b->shader, &tex->instr);
   nir_builder_instr_insert(b, original);
   nir_def *clamp_to_one = &nir_instr_as_tex(original)->def;

   nir_push_if(b, has_custom);
   nir_instr *clamped = nir_instr_clone(b->shader, &tex->instr);
   nir_builder_instr_insert(b, clamped);

   nir_tex_instr *clamp_to_zero_tex = nir_instr_as_tex(clamped);
   clamp_to_zero_tex->backend_flags |= KK_TEXTURE_FLAG_CLAMP_TO_0;
   nir_def *clamp_to_zero = &clamp_to_zero_tex->def;
   nir_def *border = kk_query_custom_border(b, clamp_to_zero_tex);
   ...
   if (nir_alu_type_get_base_type(tex->dest_type) == nir_type_float &&
      ...
      replaced = nir_flrp(b, clamp_to_zero, clamp_to_one, border);
      b->shader->info.flrp_lowered = false;
   } else {
      replaced = nir_bcsel(b, nir_ieq(b, clamp_to_zero, clamp_to_one),
                           clamp_to_zero, border);
   }

   nir_pop_if(b, NULL);
   nir_def_replace(&tex->def, nir_if_phi(b, replaced, clamp_to_one));
   return true;
}
...
{
   return nir_shader_tex_pass(nir, kk_lower_custom_border_tex, nir_metadata_none,
                              NULL);
}
